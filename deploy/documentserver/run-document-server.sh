...
# --- proposalbuilder fix: ONLYOFFICE's published image ships supervisor
# conf files with a literal, unsubstituted "COMPANY_NAME" placeholder
# (confirmed on 9.3.1.2/9.4.0/latest), which crashes supervisord before
# it starts anything. Re-apply the substitution here, immediately before
# supervisor starts, since a Dockerfile build-time sed alone didn't take
# for ds-adminpanel.conf specifically (its mtime predates our build,
# implying it isn't present/finalized at image-build time the way the
# other ds-*.conf files are).
sed -i "s/COMPANY_NAME/${COMPANY_NAME}/g" ${SUPERVISOR_CONF_DIR}/*.conf
